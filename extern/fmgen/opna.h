...
//		void	SetChannelMask(uint mask);
	...
	// ADPCMA 関係
		...
		// [FmGenEngine] この位置にあった `Channel4 ch[6];` を削除した。
		// 理由: 基底クラス OPNABase が同名・同型の protected メンバ `Channel4 ch[6]`
		// を既に持っており、この重複宣言が名前を隠蔽 (shadowing) してしまう。
		// OPNB::OPNB() コンストラクタの `csmch = &ch[2];` は本来 OPNABase::ch を指すべきところ、この隠蔽により未初期化の
		// OPNB::ch (本フィールド) を指してしまい、OPNABase::FMMix() 内の `fnum[csmch-ch]`
		// (OPNABase::ch に対するポインタ差分) が異なる配列間の演算となって
		// 未定義動作・クラッシュを引き起こす。
		// OPNB の実処理 (Mix/Mix6/MixSubS 等は全て OPNABase のメンバ関数) は
		// もともと OPNABase::ch のみを参照しており、この OPNB::ch は
		// コンストラクタの csmch 初期化以外どこからも使われていなかった
		// (デッドコード)。削除しても OPNB の機能に影響はない。
	};
