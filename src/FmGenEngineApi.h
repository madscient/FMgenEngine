...
#ifndef FMGENGINE_API_H
#define FMGENGINE_API_H
...
// =========================================================
//  [fmgen バックエンド拡張] OPNA リズムサンプル(WAV)読み込み
//  ※ YMEngine (ymfm版) には存在しない追加 API。
//     既存関数の挙動は変更していないため ABI 互換性は維持される。
//
//  OPNA (YM2608) は実機のリズム音源相当を WAV ファイル
//  (2608_BD.WAV, 2608_SD.WAV, 2608_TOP.WAV, 2608_HH.WAV,
//   2608_TOM.WAV, 2608_RIM.WAV または 2608_RYM.WAV) から読み込む。
//  dir_path にはこれらのファイルが置かれているディレクトリを
...
// =========================================================
FMENGINE_API FmResult       FMENGINE_CALL FmEngine_LoadRhythmSamples(
    FmEngineHandle engine, uint32_t chip_id, const char* dir_path);
